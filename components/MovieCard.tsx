...
const HeartIcon: React.FC<{ isFavorite: boolean }> = ({ isFavorite }) => (
  <svg
    ...
    className={`h-6 w-6 transition-all duration-300 ${isFavorite ? 'text-red-500' : 'text-white'}`}
    ...
    <Link to={`/movie/${movie.id}`} className="block group relative rounded-lg overflow-hidden shadow-lg transform transition-all duration-300 hover:scale-105 hover:shadow-xl hover:shadow-cyan-400/50">
      {currentUser && (
        <button
          ...
          className="absolute top-2 right-2 z-20 p-2 bg-black/50 rounded-full hover:bg-black/75 transition-all duration-200 hover:scale-110"
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4">
        ...
        <div className="flex justify-between items-center mt-1">
          ...
          {count > 0 && (
            <div className="flex items-center gap-1 text-sm text-yellow-400 font-bold" title={`Average rating from ${count} user${count > 1 ? 's' : ''}`}>
              <StarIcon />
